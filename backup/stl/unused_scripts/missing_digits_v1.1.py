...
def get_gap_results():
    ...
    containing the gap_value and time, results in list format
    ex. [(2, ['123', '456', '789']), ...]
    """
    ...
    for gap_value in range(2, 20):
        with open("results_v1.txt", "r") as fo:
            reversed_entries = list(islice(fo, 2, None))[::-1]
...
def main():
    for item in get_gap_results():
        ...
        if is_missing(gap_results):
            left, right, pairs, common = is_missing(gap_results)

            print("gap: {}\nresults: {}\n".format(
                gap_value, gap_results), end="")
            print("common_digit: {}\nleft: {}\n".format(
                common, left), end="")
