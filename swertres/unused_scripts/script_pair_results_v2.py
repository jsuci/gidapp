...
def pair_results():
    num_pair = input("Enter a pair to match: ")
    ...
    for entry in results:
        ...
        for item in entry:
            pair_item = match_results(item, num_pair)

            if pair_item and len(pair_item) == 2:
                new_entry.append(f"{item} ({pair_item})")
            else:
...
def export_results(results):
    ...
    with open("pair_results_v2.txt", "w") as fo:
